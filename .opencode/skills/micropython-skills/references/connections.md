...
For this WindowActuator repository, prefer the documented `firmware/build.sh`, `firmware/flash.sh`,
and `firmware/upload-src.sh` workflow. The generic `mpremote` examples below are useful for
diagnostics, but do not replace the project's custom firmware deployment process.
