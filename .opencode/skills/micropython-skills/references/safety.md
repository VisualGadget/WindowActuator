...
## WindowActuator deployment warning

The WindowActuator uses a custom frozen ESP8266 firmware. Do not use the generic
`scripts/firmware_flash.py` installer for this board: it can erase the application and replace it
with a generic image. Use the repository's `firmware/build.sh` and `firmware/flash.sh` scripts
instead. The project flash script preserves the filesystem and `settings.json`.
