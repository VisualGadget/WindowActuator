...
compatibility: Requires the project venv with esptool and rshell; mpremote is optional.
metadata:
  upstream: https://github.com/cgb-openclaw/openclaw-skills/tree/main/skills/0x1abin/micropython-skills
...
## WindowActuator project integration

This repository contains custom ESP8266 firmware and a motor-driven window actuator. For this
project, use the existing `esp8266-firmware` skill and its deployment workflow instead of the
generic firmware installer in this skill:

- Run firmware commands from the project virtual environment: `source venv/bin/activate`.
- Use `firmware/build.sh` and `firmware/flash.sh` for this project's firmware.
- Do not use `scripts/firmware_flash.py` for the WindowActuator board. It installs generic firmware
  and can erase the custom frozen application.
- `firmware/flash.sh` preserves the device filesystem and `settings.json`; do not add a full-chip
  erase unless the user explicitly requests recovery.
- The Wemos D1 mini uses `/dev/ttyUSB0`, and the project motor pins are defined in `freeze/main.py`.
- Any live motor or relay test is a cautious actuator operation. Confirm the wiring and use a safe
  movement limit before driving the load.
...
|---------------------|----------------|-------------|
| temperature, humidity, DHT, BME280, pressure, IMU, accelerometer, ADC, analog, ultrasonic, light sensor, photoresistor, read sensor | `../micropython-sensor/SKILL.md` | Safe |
| GPIO, LED, blink, PWM, servo, motor, stepper, relay, NeoPixel, WS2812, buzzer, output, control, drive | `../micropython-actuator/SKILL.md` | Cautious |
| WiFi, MQTT, HTTP, BLE, Bluetooth, NTP, WebSocket, AP mode, network, connect internet, publish, subscribe | `../micropython-network/SKILL.md` | Cautious |
| PID, filter, Kalman, state machine, scheduler, data logging, moving average, control loop, algorithm | `../micropython-algorithm/SKILL.md` | Safe |
| scan I2C, scan SPI, device info, memory, filesystem, diagnose, health check, benchmark, pin state, what's connected | `../micropython-diagnostic/SKILL.md` | Safe |
| save to device, 保存到设备, 开机自启, auto start, 下次还能用, persist, 断电保存 | See "Program Persistence" section below | Cautious/Dangerous |
| flash firmware, burn firmware, install MicroPython, no firmware, 烧录, 刷固件, 刷机 | `scripts/firmware_flash.py` (or this project's `esp8266-firmware` workflow) | Dangerous |
